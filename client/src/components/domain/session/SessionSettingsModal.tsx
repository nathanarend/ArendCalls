...
export const SessionSettingsModal = ({
  ...
  useEffect(() => {
    if (session) setWebhookUrl(session.webhookUrl || "");
  }, [session]);
    ...
    <Dialog open={!!session} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Identificação da conta e URL de Webhook de eventos.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          ...
          <div className="pt-4 border-t border-border mt-4">
            <div className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                URL HTTP para receber os eventos (Ringing, Accepted, Terminated) desta conta via POST.
                Anula a URL global do sistema. A gravação de chamadas usa outro canal, configurado em
                <strong className="text-foreground"> Configurações de Gravação</strong> (menu lateral).
              </p>
              <div className="flex flex-col gap-2">
                ...
                />
                <Button variant="secondary" onClick={handleUpdateWebhook} disabled={isSavingWebhook}>
