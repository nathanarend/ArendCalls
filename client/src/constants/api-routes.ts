...
export const sessionRoutes: RouteInfo[] = [
  ...
  { method: "POST", path: "/api/sessions/{sid}/pair", purpose: "Re-parear uma conta (emitir novo QR)", requiresSid: true },
  {
    ...
    purpose:
      "Iniciar uma chamada de saída. record: true liga a gravação no servidor desta chamada (exige o destino de gravação da instância configurado — ver seção 3). clinicId define a pasta {clinicId} na chave do bucket.",
  ...
  { method: "POST", path: "/api/sessions/{sid}/calls/{id}/webrtc", purpose: "Trocar o SDP WebRTC do navegador", payload: { sdp_offer: "v=0\no=- 123456..." }, requiresSid: true },
  {
    ...
    purpose:
      "Aceitar uma chamada de entrada. record: true grava esta chamada recebida (clinicId define a pasta no bucket). Sem record, grava só se 'gravar chamadas recebidas' estiver ligado nas Configurações de Gravação da instância.",
...
export const recordingRoutes: RouteInfo[] = [
  {
    method: "PATCH",
    path: "/api/recording-config",
    purpose:
      "Destino de gravação da INSTÂNCIA inteira (não por conta; os apps que fazem chamadas não passam nada disso). B2 e webhook são obrigatórios juntos: envie os 6 campos (b2Endpoint, b2Bucket, b2KeyId, b2AppKey, webhookUrl, webhookSecret) ou tudo vazio para limpar. Setup pela metade retorna 400 com a lista missing. PATCH parcial preserva o que não for enviado; um segredo enviado como \"\" é apagado. recordInbound: grava toda chamada recebida atendida.",
    payload: {
    ...
    response: { complete: true, b2Bucket: "gravacoes", b2AppKeySet: true, webhookSecretSet: true, urlTtlSeconds: 0, recordInbound: false },
  },
  {
    method: "GET",
    path: "/api/recording-config",
    purpose: "Ler o destino de gravação da instância. Segredos nunca são retornados — apenas b2AppKeySet / webhookSecretSet. complete indica se está pronto para entregar gravações.",
    response: { complete: false, b2Endpoint: "", b2Region: "", b2Bucket: "", b2KeyId: "", b2AppKeySet: false, b2Prefix: "", webhookUrl: "", webhookSecretSet: false, urlTtlSeconds: 0, recordInbound: false },
  },
  {
    method: "GET",
