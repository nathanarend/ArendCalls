...
export const recordingRoutes: RouteInfo[] = [
  {
    ...
    purpose:
      "Configurar o destino da gravação da conta. B2 e webhook são obrigatórios juntos: envie os 6 campos (b2Endpoint, b2Bucket, b2KeyId, b2AppKey, webhookUrl, webhookSecret) ou tudo vazio para limpar. Setup pela metade retorna 400 com a lista missing. PATCH parcial preserva o que não for enviado; um segredo enviado como \"\" é apagado.",
    payload: {
      ...
      b2AppKey: "K004...",
      b2Prefix: "clinica-x",
      webhookUrl: "https://mocho.example/api/calls/recording-ready",
      webhookSecret: "segredo-hmac-compartilhado",
      urlTtlSeconds: 900,
    },
    response: { sessionId: "SUA_SESSION_ID", complete: true, b2Bucket: "gravacoes-clinica", b2AppKeySet: true, webhookSecretSet: true, urlTtlSeconds: 900 },
    requiresSid: true,
  },
  {
    method: "GET",
    ...
    response: { sessionId: "SUA_SESSION_ID", complete: false, b2Endpoint: "", b2Region: "", b2Bucket: "", b2KeyId: "", b2AppKeySet: false, b2Prefix: "", webhookUrl: "", webhookSecretSet: false, urlTtlSeconds: 0 },
    requiresSid: true,
  },
  {
    method: "GET",
